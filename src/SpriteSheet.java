...
public class SpriteSheet{
	...
	public final int H_OFFSET = 30;
	public final int V_OFFSET = 25;
	...
	public BufferedImage grabImage(int col, int row, int width, int height){
		if(row == 1){
			...
			if(col < 10){
				...
				this.img = image.getSubimage((H_OFFSET*col) -3 +(Textures.BANGA_WIDTH*9)+(width*(col%10)), (V_OFFSET*2)+Textures.BUS_HEIGHT+(Textures.BANGA_HEIGHT-Textures.TC7_HEIGHT), width, height);

			}
			else if(col >= 13 && col < 17){
				this.img = image.getSubimage((H_OFFSET*col) -3 +(Textures.BANGA_WIDTH*9)+(Textures.TC7_WIDTH*3)+(width*(col%13)), (V_OFFSET*2)+Textures.BUS_HEIGHT+(Textures.BANGA_HEIGHT-Textures.TOWER_HEIGHT), width, height);
			}
			else if(col == 17){
				...
				this.img = image.getSubimage((H_OFFSET*col)+(Textures.BANGA_WIDTH*9)+(Textures.TC7_WIDTH*3)+(Textures.TOWER_WIDTH*5)+(Textures.ZOMBIE_WIDTH*(col%18))-10,(V_OFFSET*2)+Textures.BUS_HEIGHT, width, height);
			}
		}
			else if(row == 3){
			if(col != 7){
